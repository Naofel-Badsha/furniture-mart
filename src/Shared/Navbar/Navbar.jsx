  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition duration-300 ease-in-out  ${
        isScrolled
          ? "bg-white shadow-xl dark:text-black"
    ...
    >
      <nav className="max-w-screen-2xl container m-auto flex gap-5 items-center justify-between py-6 px-4">
        ...
        <div onClick={handleToggleMenu} className="md:hidden">
          {isMenuOpen ? null : (
            <FaBars className="text-3xl hover:text-[#E58411] cursor-pointer hover:rotate-180" />
          )}
        </div>
        ...
        >
          ...
          >
            <FaTimes className="text-3xl hover:text-rose-600 cursor-pointer hover:rotate-180" />
          </div>
